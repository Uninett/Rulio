...
def update_tag(*, actor, tenant_id, tag_id, name=None, description=None, color=None):
    require_write_tenant(actor, tenant_id)
    logger.debug(
        f"Updating tag with ID {tag_id} in tenant {tenant_id}. by actor {actor} with editable tenant IDs {_editable_tenant_ids(actor, tenant_id)}"
    )
    ...
    if tag is None:
        ...
        logger.debug(
            f"Editable tenant IDs for actor {actor} and tenant {tenant_id}: {_editable_tenant_ids(actor, tenant_id)}"
        )
