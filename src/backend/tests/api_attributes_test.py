...
def test_api_create_address(authenticated_client_with_tenant):
    ...
    address = Address.objects.filter(name="CheckThatIExist_api").first()
    assert address is not None
    assert address.name == "CheckThatIExist_api"
    assert address.ipv4Network == "192.168.0.0/24"
    assert address.ipv6Network == "2001:db8::/32"
...
def test_api_create_address_group(authenticated_client_with_tenant):
    ...
    assert response.status_code == 200
    address_group = AddressGroup.objects.filter(name="CheckThatIExistGroup_api").first()
    assert address_group is not None
    assert address_group.name == "CheckThatIExistGroup_api"
...
def test_api_add_address_to_group(authenticated_client_with_tenant):
    ...
    address_id = Address.objects.filter(name="test_api_address").first().id
    ...
    group_id = AddressGroup.objects.filter(name="test_api_group").first().id
...
def test_api_add_addresses_to_group(authenticated_client_with_tenant):
    ...
    for i in range(1, 4):
        ...
        address_id = Address.objects.get(name=f"test_api_address_{i}").id
    ...
    group_id = AddressGroup.objects.get(name="test_api_group").id
    assert group_id is not None
...
def test_api_add_addresses_to_group_invalid_group(authenticated_client_with_tenant):
    ...
    for i in range(1, 4):
        payload = {
            "name": f"test_add_address_{i}_api",
            "description": "",
            "ipv4Network": f"192.168.{i}.0/24",
            "ipv4Address_start": None,
            "ipv4Address_end": None,
        ...
        address_id = Address.objects.get(name=f"test_add_address_{i}_api").id
        addresses.append(address_id)
...
def test_api_create_and_add_address_to_group(authenticated_client_with_tenant):
    ...
    client.post(
        ...
        {"name": "test_group_api", "description": ""},
        content_type="application/json",
    )
    group_id = AddressGroup.objects.get(name="test_group_api").id
    address_payload = {
        "payload": {
            "name": "test_api_address",
            "description": "",
    ...
    address_id = Address.objects.get(name="test_api_address").id
...
def test_api_delete_address(authenticated_client_with_tenant):
    ...
    payload = {
        "name": "test_api_address_to_delete",
        "description": "",
        "ipv4Network": "192.168.0.0/24",
        "ipv4Address_start": None,
        "ipv4Address_end": None,
    ...
    address_id = Address.objects.get(name="test_api_address_to_delete").id
...
def test_api_create_service(authenticated_client_with_tenant):
    ...
    service_id = Service.objects.filter(name="test_api_service").first().id
...
def test_api_create_service_group(authenticated_client_with_tenant):
    ...
    group_id = ServiceGroup.objects.filter(name="test_api_service_group").first().id
    assert group_id is not None

    group = ServiceGroup.objects.get(id=group_id)
    assert group.name == "test_api_service_group"
...
def test_api_add_services_to_group(authenticated_client_with_tenant):
    ...
    for i in range(1, 4):
        ...
        service_id = Service.objects.get(name=f"test_api_service_{i}").id
    ...
    group_id = ServiceGroup.objects.get(name="test_api_service_group").id
    assert group_id is not None
...
def test_api_create_and_add_service_to_groups(authenticated_client_with_tenant):
    ...
    client.post(
        ...
        {"name": "test_api_service_group", "description": ""},
        content_type="application/json",
    )
    group_id = ServiceGroup.objects.get(name="test_api_service_group").id
    service_payload = {
    ...
    service_id = Service.objects.get(name="test_api_service").id
...
def test_api_delete_service(authenticated_client_with_tenant):
    ...
    payload = {
        "name": "test_api_service_to_delete",
        "description": "",
        "protocol": "tcp",
        "port_start": 80,
        "port_end": 80,
    ...
    service_id = Service.objects.get(name="test_api_service_to_delete").id
...
def test_api_delete_service_group(authenticated_client_with_tenant):
    ...
    payload = {
        "name": "test_api_service_group_to_delete",
        "description": "",
    }
    response = client.post("/api/create_service_group", data=json.dumps(payload), content_type="application/json")
    assert response.status_code == 200

    group_id = ServiceGroup.objects.get(name="test_api_service_group_to_delete").id
...
def test_api_create_tag(authenticated_client_with_tenant):
    ...
    tag_id = Tag.objects.filter(name="test_api_tag").first().id
...
def test_api_create_and_add_tag_to_object(authenticated_client_with_tenant):
    ...
    tag_payload = {"object_type": "address", "object_id": address_id, "name": "test_api_tag", "description": ""}
    response = client.post(
