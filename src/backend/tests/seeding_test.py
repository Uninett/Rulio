...
def test_populate_db_is_idempotent_for_seed_data():
    ...
    address_count_before_seeding = Address.objects.filter(
        tenant_id=GLOBAL_TENNANT_ID
    ).count()
    service_count_before_seeding = Service.objects.filter(
        tenant_id=GLOBAL_TENNANT_ID
    ).count()
    ...
    address_count_after_first_seeding = Address.objects.filter(
        tenant_id=GLOBAL_TENNANT_ID
    ).count()
    service_count_after_first_seeding = Service.objects.filter(
        tenant_id=GLOBAL_TENNANT_ID
    ).count()
    ...
    address_count_after_second_seeding = Address.objects.filter(
        tenant_id=GLOBAL_TENNANT_ID
    ).count()
    service_count_after_second_seeding = Service.objects.filter(
        tenant_id=GLOBAL_TENNANT_ID
    ).count()
